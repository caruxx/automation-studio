...
**単一の Web ダッシュボード (localhost:8888) で 6 工程管理**する。
...
master_settings.json     # v2: SUNO 詳細設定・Flow 枚数・メタ目標値・Tunnel URL
...
| 工程 | UI 起点 | 完了判定ファイル | 担当スクリプト |
|------|---------|----------------|--------------|
| SUNO生成 | SUNO タブ / 楽曲タブ | `music/*.mp3` が 1 本以上 | suno_auto_create.py |
| 画像選択 | 画像タブ | `selected_images.json` or `vol{N}.jpg` | （Web 内完結） |
| JSX配置 | 書き出しタブ | `subtitles_*.srt` | app_premiere.py |
| 書き出し | 書き出しタブ | `vol_vol*.mp4` | app_premiere.py `--export` |
| 動画メタ | 詳細タブ | `youtube_description.txt` + `youtube_tags.txt` | claude_proposer.py |
| アップロード | アップロードタブ | `youtube_upload.json` | app_youtube.py |

判定ロジックは [index.html](web/static/index.html) の `WORKFLOW_STEPS` 定数で定義。
...
| [app-master-config.md](skills/app-master-config.md) | **v2** マスター設定タブ（9 セクションでプロンプト / SUNO/Flow / スケジュール / リモート / 入出力を集約） |
