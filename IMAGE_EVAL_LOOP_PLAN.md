...
## 評価設計の確定判断（2026-06-01 相談）

「ベンチと似てる/刺さってる」の数値化・データ蓄積について、3点を確定:

1. **類似度の数値化 = 第1版は既存 Vision スコアのみ**（score_total / ctr_predict / similarity）。
   - 当初 CLIP embedding 距離のハイブリッドを推奨したが、**この環境は素の Python 3.9.6 で PIL/numpy も未導入**、画像処理は sips 等の外部コマンドに委譲する設計。torch+CLIP（数百MB〜GB）は重い導入になるため第1版では見送り。
   - 「似てる」は逆向き2義に注意: (a)**遜色なさ**=ベンチに近い方が良い（score_total/ctr_predict が測る）、(b)**パクリ度**=固有要素コピー＝遠い方が良い（similarity_max=25 の上限ガード）。similarity は機械距離でなく Vision の主観判定（固有要素コピー度、低いほど良い）。
   - 将来 CLIP/OpenAI Embeddings で「最適距離帯」を機械的・冪等に測る軸を**動くものを見てから別途追加**（段階 S5 候補）。
2. **「刺さり」の真の教師信号 = 実績（ab_log の views_delta_7d）だが7日遅延・手動転記**。生成ループ（即時）では使えないため**別レイヤに分離**。生成時は Vision の strengths/comment/ctr_predict（予測）で代理判定。実績突合・閾値の実績学習は**生成ループが安定してから**（段階 S6 候補）。
3. **データ蓄積 = 第1版は JSON（thumbnail_state.json / ab_log.json のまま）**。横断分析・実績突合・学習をやる中期以降に SQLite（runs.db のように）。最初から全部 DB にすると複雑で頓挫するため段階的に。
