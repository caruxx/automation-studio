...
- 主張には可能な限り根拠（どの動画・どの数字か）を添え、データで裏付かない推測は推測として明示してください。
- 「伸びた要因」を挙げる前に、投稿タイミング、ジャンル全体の波、登録者初速、外部流入、単なる分散などの代替説明を検討してください。
- 再生数は生の値だけで判断せず、同一チャンネル内の上位/直近/中央値に対する相対比較として扱ってください。
...
def propose_with_analysis(
    ...
    try:
        ...
        seed_block = _seed.seed_prompt_hint()
    except Exception:
        seed_block = ""
    seed_section = f"=== Seed Video Analysis (1本単位の深掘り) ===\n{seed_block}" if seed_block else ""
...
def propose_suno_prompt(
    ...
    try:
        ...
        seed_block = ""
    seed_section = f"=== Seed Video Analysis（音楽方向の参考。コピー禁止） ===\n{seed_block}" if seed_block else ""
    seed_music_block = ""
    try:
        import app_benchmark_seed as _seed
        seed_music_block = _seed.seed_music_profile_hint()
    except Exception:
        seed_music_block = ""
    seed_music_section = f"=== ベンチマーク音源分析(Gemini実聴) ===\n{seed_music_block}" if seed_music_block else ""
...
- 音源分析がある場合、BPM・楽器・密度・質感はそれを最優先の根拠にする
    ...
    if url_list:
        ...
        for i, url in enumerate(url_list):
            try:
                ...
                with urllib.request.urlopen(url, timeout=60) as response, dst.open("wb") as out:
